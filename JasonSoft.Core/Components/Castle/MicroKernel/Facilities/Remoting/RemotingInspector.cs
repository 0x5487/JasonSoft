...
namespace JasonSoft.Components.Castle.Facilities.Remoting
{
    ...
    public class RemotingInspector : IContributeComponentModelConstruction
    {
                ...
                model.LifestyleType != LifestyleType.Undefined)
            {
                throw new FacilityException(
                    String.Format("Component {0} is marked as a 'RecoverableComponent' but is using a lifestyle " +
                                  "different than Singleton. Unfortunatelly Singleton is the only lifestyle supported for this of remoting component configuration",
                                  model.Name));
            }
        }
        ...
        private void ConfigureClientComponent(RemotingStrategy client, Type type, ComponentModel model)
        {
            ...
            bool skipRemotingRegistration =
                Convert.ToBoolean(model.Configuration.Attributes["skipRemotingRegistration"]);
