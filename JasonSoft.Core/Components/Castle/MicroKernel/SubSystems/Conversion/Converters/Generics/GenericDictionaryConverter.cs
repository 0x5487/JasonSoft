        ...
        public override object PerformConversion(IConfiguration configuration, Type targetType)
        {
            ...
            IGenericCollectionConverterHelper collectionConverterHelper =
                ...
                Activator.CreateInstance(
                    typeof (DictionaryHelper<,>).MakeGenericType(defaultKeyType, defaultValueType), this);
            ...
            public object ConvertConfigurationToCollection(IConfiguration configuration)
            {
                ...
                foreach (IConfiguration itemConfig in configuration.Children)
                {
                    ...
                    if (itemConfig.Attributes["keyType"] != null)
                    {
                        convertKeyTo =
                            (Type)
                            parent.Context.Composition.PerformConversion(itemConfig.Attributes["keyType"], typeof (Type));
                    }
                    ...
                    if (!typeof (TKey).IsAssignableFrom(convertKeyTo))
                    {
                        throw new ArgumentException(
                            string.Format(
                                "Could not create dictionary<{0},{1}> because {2} is not assignmable to key type {0}",
                                typeof (TKey),
                    ...
                    if (itemConfig.Attributes["valueType"] != null)
                    {
                        convertValueTo =
                            (Type)
                            parent.Context.Composition.PerformConversion(itemConfig.Attributes["valueType"],
                                                                         typeof (Type));
                    }
                    ...
                    if (!typeof (TValue).IsAssignableFrom(convertValueTo))
                    {
                        throw new ArgumentException(
                            string.Format(
                                "Could not create dictionary<{0},{1}> because {2} is not assignmable to value type {1}",
