...
namespace JasonSoft.Components.Castle.MicroKernel.ModelBuilder.Inspectors
{
    /// <summary>
    ...
    public class ComponentProxyInspector : IContributeComponentModelConstruction
    {
        ...
        protected virtual void ReadProxyBehavior(IKernel kernel, ComponentModel model)
        {
            ...
            string useSingleInterfaceProxyAttrib = model.Configuration != null
                                                       ? model.Configuration.Attributes["useSingleInterfaceProxy"]
                                                       : null;
            string marshalByRefProxyAttrib = model.Configuration != null
                                                 ? model.Configuration.Attributes["marshalByRefProxy"]
                                                 : null;
                ...
                try
                {
                    proxyBehaviorAtt.UseSingleInterfaceProxy = (bool)
                                                               converter.PerformConversion(
                                                                   useSingleInterfaceProxyAttrib, typeof (bool));
                }
                catch (ConverterException ex)
                {
                    throw new ConfigurationErrorsException("Could not convert attribute " +
                                                           "'useSingleInterfaceProxy' to bool. Value is " +
                                                           useSingleInterfaceProxyAttrib, ex);
                }
            }
            ...
            if (marshalByRefProxyAttrib != null)
            {
                try
                {
                    proxyBehaviorAtt.UseMarshalByRefProxy = (bool)
                                                            converter.PerformConversion(marshalByRefProxyAttrib,
                                                                                        typeof (bool));
                }
                catch (ConverterException ex)
                {
                    throw new ConfigurationErrorsException("Could not convert attribute " +
                                                           "'marshalByRefProxy' to bool. Value is " +
                                                           marshalByRefProxyAttrib, ex);
                }
            }
