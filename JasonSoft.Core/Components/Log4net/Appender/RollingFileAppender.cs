...
namespace JasonSoft.Components.Log4net.Appender
{
    ...
    public class RollingFileAppender : FileAppender
    {
            ...
            lock (this)
            {
                ...
                else
                {
                    if (LogLog.IsErrorEnabled)
                    {
                        ...
                        {
                            LogLog.Error("RollingFileAppender: INTERNAL ERROR. Append is False but OutputFile [" +
        ...
        private void RollOverIfDateBoundaryCrossing()
        {
            if (m_staticLogFileName && m_rollDate)
            {
                if (FileExists(m_baseFileName))
                {
                    ...
                    if (
                        !(last.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo).Equals(
                             m_now.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo))))
                    {
        ...
        private void InitializeRollBackups(string baseFile, ArrayList arrayFiles)
        {
            if (null != arrayFiles)
            {
                ...
                {
                    InitializeFromOneFile(baseFileLower,
                                          curFileName.ToLower(System.Globalization.CultureInfo.InvariantCulture));
                }
            }
        }
