...
using JasonSoft.Components.Log4net.ObjectRenderer;
using JasonSoft.Components.Log4net.Util;

namespace JasonSoft.Components.Log4net.Repository.Hierarchy
{
    /// <summary>
    ...
    public class XmlHierarchyConfigurator
    {
            ...
            if (thresholdStr.Length > 0 && thresholdStr != "null")
            {
                ...
                else
                {
                    LogLog.Warn("XmlHierarchyConfigurator: Unable to set hierarchy threshold using value [" +
                                thresholdStr + "] (with acceptable conversion types)");
                }
            }
        ...
        protected IAppender ParseAppender(XmlElement appenderElement)
        {
            ...
            {
                IAppender appender =
                    (IAppender) Activator.CreateInstance(SystemInfo.GetTypeFromString(typeName, true, true));
            ...
            else
            {
                ...
                else
                {
                    LogLog.Debug("XmlHierarchyConfigurator: Logger [" + loggerName + "] level set to [name=\"" +
                                 log.Level.Name + "\",value=" + log.Level.Value + "].");
                }
            }
        }
        ...
        protected void SetParameter(XmlElement element, object target)
        {
            ...
            {
                LogLog.Error("XmlHierarchyConfigurator: Cannot find Property [" + name + "] to set object on [" +
                             target.ToString() + "]");
            }
            else
            {
                ...
                if (propertyValue != null)
                {
                    ...
                    try
                    {
                        ...
                        propertyValue =
                            OptionConverter.SubstituteVariables(propertyValue, Environment.GetEnvironmentVariables());
                    }
                            ...
                            if (!propertyType.IsAssignableFrom(subType))
                            {
                                ...
                                else
                                {
                                    LogLog.Error("XmlHierarchyConfigurator: Subtype [" + subType.FullName + "] set on [" +
                                                 name + "] is not a subclass of property type [" + propertyType.FullName +
                    ...
                    else
                    {
                        LogLog.Warn("XmlHierarchyConfigurator: Unable to set property [" + name + "] on object [" +
                                    target + "] using value [" + propertyValue + "] (with acceptable conversion types)");
                    }
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Got a converted result
                            LogLog.Debug("XmlHierarchyConfigurator: Setting Property [" + propInfo.Name +
                        ...
                        else if (methInfo != null)
                        {
                            ...
                            {
                                LogLog.Error(
                                    "XmlHierarchyConfigurator: Failed to set parameter [" + methInfo.Name +
                                    "] on object [" + target + "] using value [" + createdObject + "]",
                                    targetInvocationEx.InnerException);
                            }
                        }
        ...
        private MethodInfo FindMethodInfo(Type targetType, string name)
        {
            ...
            foreach (MethodInfo methInfo in methods)
            {
                ...
                {
                    if (
                        string.Compare(methInfo.Name, requiredMethodNameA, true,
                                       System.Globalization.CultureInfo.InvariantCulture) == 0 ||
                        string.Compare(methInfo.Name, requiredMethodNameB, true,
                                       System.Globalization.CultureInfo.InvariantCulture) == 0)
                    {
        ...
        protected object CreateObjectFromXml(XmlElement element, Type defaultTargetType, Type typeConstraint)
        {
            ...
            catch (System.Exception createInstanceEx)
            {
