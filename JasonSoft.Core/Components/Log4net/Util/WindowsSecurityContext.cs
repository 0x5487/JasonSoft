...
namespace JasonSoft.Components.Log4net.Util
{
    /// <summary>
    ...
    public class WindowsSecurityContext : SecurityContext, IOptionHandler
    {
        ...
        private static WindowsIdentity LogonUser(string userName, string domainName, string password)
        {
                           ...
                           ref tokenHandle))
            {
                ...
                throw new System.Exception("Failed to LogonUser [" + userName + "] in Domain [" + domainName +
            ...
            if (!DuplicateToken(tokenHandle, SecurityImpersonation, ref dupeTokenHandle))
            {
                ...
                throw new System.Exception("Failed to DuplicateToken after LogonUser. Error: " + error.ToString());
            }
        ...
        private static extern bool DuplicateToken(IntPtr ExistingTokenHandle, int SECURITY_IMPERSONATION_LEVEL,
