...
using JasonSoft.Components.Log4net.Plugin;
using JasonSoft.Components.Log4net.Repository;
using JasonSoft.Components.Log4net.Repository.Hierarchy;
using JasonSoft.Components.Log4net.Util;

namespace JasonSoft.Components.Log4net.Core
{
    /// <summary>
    ...
    public class DefaultRepositorySelector : IRepositorySelector
    {
                    ...
                    if (aliasedRepository != null)
                    {
                        ...
                        else
                        {
                            ...
                            LogLog.Error("DefaultRepositorySelector: Failed to alias repository [" + repositoryName +
                                         "] to existing repository [" + aliasedRepository.Name +
                                         "]. Requested repository type [" + repositoryType.FullName +
                    ...
                    if (repositoryTarget != ((ILoggerRepository) m_alias2repositoryMap[repositoryAlias]))
                    {
                        ...
                        throw new InvalidOperationException("Repository [" + repositoryAlias +
                                                            ...
                                                            ((ILoggerRepository) m_alias2repositoryMap[repositoryAlias])
                                                                .Name + "]. Aliases cannot be redefined.");
                    }
                }
        ...
        private void GetInfoForAssembly(Assembly assembly, ref string repositoryName, ref Type repositoryType)
        {
            ...
            try
            {
                ...
                object[] repositoryAttributes =
                    Attribute.GetCustomAttributes(assembly, typeof (RepositoryAttribute), false);
        ...
        private void LoadPlugins(Assembly assembly, ILoggerRepository repository)
        {
            ...
            if (configAttributes != null && configAttributes.Length > 0)
            {
                foreach (IPluginFactory configAttr in configAttributes)
                {
                    ...
                    {
                        LogLog.Error(
                            "DefaultRepositorySelector: Failed to create plugin. Attribute [" + configAttr.ToString() +
                            "]", ex);
                    }
                }
            }
        ...
        private void LoadAliases(Assembly assembly, ILoggerRepository repository)
        {
            ...
            object[] configAttributes =
                Attribute.GetCustomAttributes(assembly, typeof (AliasRepositoryAttribute), false);
            if (configAttributes != null && configAttributes.Length > 0)
            {
