...
namespace JasonSoft.Web.Compression
{
    public sealed class EncodingManager
    {
        ...
        private readonly string _RequestHeader = string.Empty;
        private readonly HttpContext _currentContext;
        internal EncodingManager(HttpContext context)
        {
            _currentContext = context;
            if( !string.IsNullOrEmpty(context.Request.Headers["Accept-encoding"]))
            {
                _RequestHeader = context.Request.Headers["Accept-encoding"].Replace(" ", string.Empty);
            }
        ...
        internal EncodingType PreferredEncodingType
        {
        ...
        /// </summary>
        internal void CompressResponse()
        {
        ...
        /// <returns></returns>
        internal byte[] CompressBytes(byte[] buffer)
        {
        ...
        /// </summary>
        internal void SetResponseEncodingType()
        {
            if (IsEncodingEnabled)
            {
                _currentContext.Response.AppendHeader("Content-encoding", PreferredEncoding);
            }
        }  
    }
}
